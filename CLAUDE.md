...
- 法令タブ 7 つ（`LAWS` 配列）＋「関連法 ▾」ドロップダウン（`SUBLAWS` 配列、19 法・分野別グループ）。追加はどちらかの配列に law_id を足すだけ
- **未施行の新法フォールバック**（`buildParsed`）: 現行版の本則が 0 条なら `/law_revisions` から直近の未施行リビジョンを取得し「⚠️ 未施行（施行予定日）」バッジ付きで表示。施行日が来て現行版が切り替わると自動で通常表示になる（改正チェックは `baseRevId` = law_id 取得版のリビジョン ID で比較）。例: 譲渡担保法（令和7年法律56号、本則施行 2027-12-05 予定）
